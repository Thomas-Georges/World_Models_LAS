...
from wm_poc.dino_wm.val_no_grad_patch import PATCH_MARKER, patch_train_file  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Patch upstream DINO-WM train.py to run validation under torch.no_grad()."
    )
...
def main() -> int:
    ...
    if args.verify_only:
        if PATCH_MARKER not in train_path.read_text(encoding="utf-8"):
            print(f"DINO-WM val no-grad patch is not applied: {train_path}")
            return 1
        print(f"DINO-WM val no-grad patch verified: {train_path}")
        return 0

    if patch_train_file(train_path):
        print(f"Applied DINO-WM val no-grad patch: {train_path}")
    else:
        print(f"DINO-WM val no-grad patch already applied: {train_path}")
    return 0
